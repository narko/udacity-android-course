...
public class SettingsFragment extends PreferenceFragmentCompat implements
    ...
    public boolean onPreferenceChange(Preference preference, Object newValue) {
        Toast errorMessage = Toast.makeText(getContext(), getString(R.string.pref_size_error_message) ,Toast.LENGTH_SHORT);
        if (preference.getKey().equals(getString(R.string.pref_size_key))) {
            String inputSize = (String) newValue;
            try {
                float size = Float.parseFloat(inputSize);
                if (size <= 0 || size > 3) {
                    errorMessage.show();
                    return false;
                }
